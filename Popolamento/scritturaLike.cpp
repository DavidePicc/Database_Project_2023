...
int main() {
    ...
    ofstream outputFile("outputLike.txt"); // Apriamo il file di output
    ...
    for (int i = 0; i < tuple; ++i) {
        outputFile << "('" << PQgetvalue(res, i, 1) << "', '" << PQgetvalue(res, i, 2) << "', '" << PQgetvalue(res, i, 3) << "', '" << PQgetvalue(res, i, 4)  << "')," << endl;
    }
