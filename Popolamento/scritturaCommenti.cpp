...
int main() {
    ...
    ofstream outputFile("outputCommenti.txt"); // Apriamo il file di output
    ...
    PGresult* res;
    res = PQexec(conn, "SELECT * FROM commenti");
    ...
    for (int i = 0; i < tuple; ++i) {
        outputFile << "('" << PQgetvalue(res, i, 1) << "', '" << PQgetvalue(res, i, 2) << "', '" << PQgetvalue(res, i, 3) << "', '" << PQgetvalue(res, i, 4)  << "', '" << PQgetvalue(res, i, 5)  << "', NULL"  << ")," << endl;
    }
